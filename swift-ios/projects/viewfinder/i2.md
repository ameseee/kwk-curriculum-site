...
* On the next line, write `cell.textLabel?.text = "anything you want in here!"` Let's break this down:
  - `cell` access the instance of the `UITableViewCell`.
...
* Before we add any code, we need to create add an Entity with Core Data. Follow the directions in the screenshot below.
